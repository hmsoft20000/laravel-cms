...
class CmsRouteManager
{
     ...
     * @deprecated Use the `nestedResource` method instead.
     */
    public function nestedResource0(string $childName, string $parentPluralName, string $parentBindingName, ?Closure $callback = null): void
    {
    ...
    /**
     * Registers a nested resource group using a dynamic owner binding name.
     * يقوم بتسجيل مجموعة موارد متداخلة باستخدام اسم ربط مالك ديناميكي.
     */
    public function nestedResource(string $childName, string $parentPluralName, ?Closure $callback = null): void
    {
        // 1. إنشاء Blueprint مبدئيًا بدون prefix
        $defaults = [
            'file' => Str::plural($childName) . ".php",
            'as' => "api.{$parentPluralName}." . Str::plural($childName) . ".",
            'middleware' => ['api'],
        ];
        ...
        // 2. تحديد اسم متغير الربط (الافتراضي هو 'owner')
        $bindingName = $blueprint->getOwnerBinding() ?? 'owner';

        // 3. بناء البادئة النهائية ديناميكيًا
        $blueprint->prefix("{$parentPluralName}/{{$bindingName}}/" . Str::plural($childName));

        // 4. استدعاء الدالة المركزية للتسجيل (بدون تمرير callback مرة أخرى)
        $this->registerRouteGroup($blueprint);
    }
