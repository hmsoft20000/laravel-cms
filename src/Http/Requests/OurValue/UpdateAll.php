...
class UpdateAll extends MyRequest
{
    ...
    {
        $file = 'cms.our_values.validation.update_all.messages';
        return is_array(trans($file)) ? trans($file) : [];
    }

    public function attributes()
    {
        $file = 'cms.our_values.validation.update_all.attributes';
        return is_array(trans($file)) ? trans($file) : [];
    }
                ...
                if (isset($item['locales'])) {
                    ...
                    if (! $hasAtLeastOneName) {
                        $validator->errors()->add(
                            "{$index}.locales.*.title",
                            trans('cms.our_values.validation.update_all.at_least_one_title')
                        );
                    }
                }
            }
