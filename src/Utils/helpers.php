...
if (!function_exists('formatDescription')) {
    function formatDescription($description, $maxLength = 100)
    {
        ...
        // return $service->truncate($description, $maxLength, '...', true);
    }
}
...
if (!function_exists('storageDisk')) {
    ...
    function storageDisk(?string $disk = null): \Illuminate\Filesystem\FilesystemAdapter
    {
...
if (!function_exists('formatNumber')) {
    ...
    function formatNumber(string $value): array
    {
        $number = (int) $value;
        $suffix = '';
        ...
        if ($number >= 1000000000) {
            $formatted_value = round($number / 1000000000, 1);
            $suffix = 'B';
        } elseif ($number >= 1000000) {
            $formatted_value = round($number / 1000000, 1);
            $suffix = 'M';
        } elseif ($number >= 1000) {
            $formatted_value = round($number / 1000, 1);
            $suffix = 'K';
        }
